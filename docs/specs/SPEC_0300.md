...
**Codex's adversarial review (kept outside the repository, like every review brief and review from v0.33.0) was checked finding by finding against the code. Eight held and are in this spec; two held with a different fix; one was rejected on wrong evidence; one led to a larger cut. Two more problems were found while checking it.**
...
**Codex's second review (kept outside the repository) found four real gaps and pushed back on three decisions; the owner took all of it on 21 September 2026, including building encrypted archives into this release.** The gap Codex called fundamental, the backup share, is larger than it described: an unencrypted archive is a readable copy of the whole collection outside the login, not only an alterable one.
...
**Codex's fourth review (kept outside the repository) returned "approve, with rules to add during the build": no critical or high finding, and every round-3 item closed.** That meets the stopping rule the owner set before the round.
...
("approve, no findings"; the report is kept outside the repository), checked by a Fable
...
## Share routes (v0.32.0)

Added by [SPEC_0320](SPEC_0320.md) (stage 1, amended by stage 4), which makes 127 operations in all. `share` is a class of its own: anyone may call these routes, and while `share_enabled` is on the gate looks up no credential for a `GET` or `HEAD` under `/api/share/` (a cookie never touches `last_seen_at`, a token is never validated, and `Sec-Fetch-Site` isn't checked), and layer 2 ignores whatever principal there is, so a signed-in admin sees what a stranger sees. Any other method under the prefix is handled as before. While `share_enabled` is off (held in memory, so the gate reads no database for it) the gate has no share rule at all: an anonymous request there is `401` like any unlisted path. While it is on, a malformed, unknown, or revoked token answers `404 {"detail": "Not found"}`; only such a failed lookup reaches the throttle (`share:<ip>`, an IPv6 address by its /64, in a map of its own; 429 inside an address's wait past 20, or past 300 failed lookups a minute from all addresses, not counted), so a live link is never throttled.

| Method | Path | Class | Note |
|---|---|---|---|
| GET | `/api/share/{token}` | share | The manifest (kind, name, options, piece count, a checklist's filled of total); counts an open |
| GET | `/api/share/{token}/items` | share | `offset`, `limit` 1 to 100; each piece through the allowlist |
| GET | `/api/share/{token}/items/{item_id}` | share | 404 unless the piece is in the share |
| GET | `/api/share/{token}/checklist` | share | Filled slots only; 404 for the other kinds |
| GET | `/api/share/{token}/photos/{photo_id}/{variant}` | share | `thumb` or `full`; 404 unless `show_photos` and the photo's piece is in the share |
| GET | `/api/share-links` | admin | Every link; never a token or its hash |
| POST | `/api/share-links` | admin, fresh | The URL shown once; 409 while sharing is off or with 20 links; audited and alerted |
| PATCH | `/api/share-links/{link_id}` | admin, fresh | Name and options only; audited with what changed, alerted when notes, values, or cert numbers go on |
| POST | `/api/share-links/{link_id}/regenerate` | admin, fresh | A new URL, shown once; the old one dead; 409 while sharing is off; audited and alerted |
| DELETE | `/api/share-links/{link_id}` | admin, fresh | Revokes for good (deletes the row); audited and alerted |

## Single sign-on routes (v0.33.0)

Added by [SPEC_0330](SPEC_0330.md) (stages 2 and 3), which makes 137 operations in all. The start route is an ordinary anonymous pair in the gate; a session sent with it is looked up, since linking and confirming need one. The callback is passed by layer 1 with no credential lookup at all (CR-01): it is a cross-site top-level navigation from the provider that a Lax session cookie rides along on, so no session is read or touched and no CSRF decision is made there; its CSRF protection is the `state` in the encrypted, one-use flow cookie, and the handler binds a link or a confirm to the session that started it by the hash of its secret. Failed callbacks are throttled per address (`oidc:<ip>`, an IPv6 address by its /64, in a map of their own, with a global 300 a minute), counting failures only.

| Method | Path | Class | Note |
|---|---|---|---|
| GET | `/api/auth/oidc/start` | public | `provider=<id>`, `next`, `intent` (`login`, `link`, `confirm`); 302 to the provider with the flow cookie; 404 for an unknown or disabled provider; `link` needs a fresh session, `confirm` a live `oidc` session from this provider that can confirm (403 otherwise, CR-07) |
| GET | `/api/auth/oidc/callback` | public | Layer 1 passes it with no credential lookup (CR-01); 303 to `next`, `/login?error=`, `/settings/signin`, or `next` with `confirm_error`; one-use flow cookie and state; failures throttled in their own map |
| GET | `/api/auth/signin-config` | admin | The providers (never a secret), the linked identities, the switches, the presets, and the callback URLs |
| PUT | `/api/auth/signin-config` | admin, fresh | The password sign-in alert switch, or the trusted-header mode switched back on (R2-02); audited (`sso_configured`) |
| POST | `/api/auth/providers` | admin, fresh | Add a provider from a preset or a custom issuer; `dry_run` fetches discovery only; 409 with two `PUBLIC_ORIGINS` on one host (R2-07); audited and alerted |
| PATCH | `/api/auth/providers/{provider_id}` | admin, fresh | A secret is replaced, never read back; `issuer`, `client_id`, `kind`, `preset` are 409 while an identity is linked (R2-13); switching it off ends its sessions; audited and alerted |
| DELETE | `/api/auth/providers/{provider_id}` | admin, fresh | Removes it and its linked identities after ending their sessions; audited and alerted |
| DELETE | `/api/auth/identities/{identity_id}` | admin, fresh | Unlink; its sessions end; audited and alerted |
| POST | `/api/auth/trusted` | public | The gateway's assertion in the header `TRUSTED_ASSERTION_HEADER` names, verified here and on the link route only (R2-03); no body, 8 KiB at most, `Sec-Fetch-Site` cross-site or same-site refused; 404 while the mode is unconfigured or switched off (R2-02); 403 `assertion` or `unlinked`, audited and throttled in the `oidc` map; 502 when the gateway's keys can't be fetched; a session with no device cookie |
| POST | `/api/auth/identities/trusted_header` | admin, fresh | Link the identity the current request's assertion names; 404 while the mode is off; 403 `assertion` or `subject` (R2-06); 409 `already_linked`; audited and alerted |
