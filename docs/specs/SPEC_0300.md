...
Added by [SPEC_0320](SPEC_0320.md) (stage 1), which makes 127 operations in all. `share` is a class of its own: anyone may call these routes, the gate looks up no credential for a `GET` or `HEAD` under `/api/share/` (a cookie never touches `last_seen_at`, a token is never validated, and `Sec-Fetch-Site` isn't checked), and layer 2 ignores whatever principal there is, so a signed-in admin sees what a stranger sees. Any other method under the prefix is handled as before. While `share_enabled` is off, and for a malformed, unknown, or revoked token, every share route answers `404 {"detail": "Not found"}`, counted against the address (`share:<ip>`, 429 past 20).
...
| PATCH | `/api/share-links/{link_id}` | admin | Name and options only |
