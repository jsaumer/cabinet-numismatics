...
Status: draft awaiting the owner's approval, 21 September 2026; nothing is built. Read [SPEC_0300-how-it-works.md](SPEC_0300-how-it-works.md) first for the plain-language tour. The owner's working copy is a claude.ai doc; this file is the snapshot reviewers read.

## Status and scope

**Draft for the owner's approval. No code, branch, migration, or commit of code until it is approved.** This is the contract for v0.30.0, roadmap Phase 7, P8, part A1: one admin, database-backed sessions, scoped API tokens, a deny-by-default gate, photos behind the same check, and the three approved API renames. **For a plain-language tour of setup, sign-in, password changes, and the break-glass reset, read SPEC_0300-how-it-works.md first.** Sections 12 and 13 record Codex's two adversarial reviews of 21 September 2026 and what was taken from each; the largest result is that every backup archive is now encrypted (section 2).
...
| `api_tokens` | `id`, `public_id` String(10) unique, `secret_hash` bytes(32), `user_id` FK, `name` String(100), `scope` String(8) `CHECK IN ('read','write','metrics')`, `created_at`, `last_used_at`, `expires_at` (required for `read` and `write`), `revoked_at` | One scope per token |
...
**Every archive Cabinet writes is encrypted, with no plain-text option, because an archive is the whole collection: every item, value, storage location, photo, and receipt.** Until now archives on the backup share (an NFS bind on the owner's Swarm) were a readable copy of the burglar's map outside the login. Codex's round 2 raised the integrity half (an archive on a writable share can be altered); confidentiality is the larger half.
...
| The key | One age identity (`AGE-SECRET-KEY-1...`). From `BACKUP_KEY_FILE` (a Docker secret) if set; otherwise generated at first start into `backup.key` (mode 0600) on the state volume. More than one identity in the file is allowed for rotation: the first encrypts, all are tried to decrypt |
...
| Tamper evidence | age authenticates the contents, but anyone who knows the public recipient could make a new archive. So the manifest carries `mac`: HMAC-SHA256 over `SHA256SUMS`, keyed by HKDF-SHA256(identity, info `cabinet-backup-mac-v1`). Restore decrypts, then verifies `mac` before reading anything else; a mismatch is 422 "This archive was not made with your backup key." Only someone with the private key can forge an archive |
| Every path | Download, scheduled, run-now, pre-restore safety backups, and `backup.sh` all encrypt. In-app restore and `restore.sh` decrypt and verify |
| `restore.sh` | Decrypts with the running backend's key (`docker compose exec -T backend python -m app.cli decrypt-archive < in > out`), or with `age -d -i <key file>` on the host for a new machine, then verifies `mac` through the same CLI before touching anything |
| Old archives | Plain `.zip` archives already on the share are listed as **unencrypted** in Settings, with a warning and a "Delete unencrypted archives" action (fresh). A plain `.zip` can still be restored after a typed `RESTORE UNENCRYPTED` confirmation, for a pre-v0.30.0 archive; the alert and audit record it |
...
| Rotation | `python -m app.cli backup-key rotate` adds a new identity first in the file (or the owner updates the Docker secret); older archives stay readable while the old identity stays in the file |
| Data volumes | Out of Cabinet's reach: Postgres's data directory, the photo volume, and the document volume are plain files. `docs/deployment.md` gains NAS hardening: export only to the Swarm nodes, keep `root_squash`, never share those folders over SMB or to other hosts |

Route paths do not change (`GET /api/backup.zip` still downloads, now named `...zip.age` in `Content-Disposition`), and `GET /api/backups` gains `encrypted: bool` per archive plus the key's `fingerprint` and `saved`. Two routes are new, both admin: `POST /api/backups/key/saved` (the "I have saved it" tick, audited) and `DELETE /api/backups/unencrypted` (fresh; deletes every plain `.zip` archive in `BACKUP_DIR`, never a `.restore-*` entry, audited and alerted). Inspect decrypts an uploaded or stored archive into `.restore-staging/` and verifies `mac` before reading the manifest, so the summary never describes an archive it could not authenticate. The CI drills and the Playwright restore test use encrypted archives.

**Where the key lives matters as much as the encryption.** A generated key on the state volume is only as private as that volume, and on the owner's Swarm the data volumes are NFS binds. So: `BACKUP_KEY_FILE` from a Docker secret (held encrypted in the Swarm's Raft store, never on the share) is the documented path for a Swarm; and at startup the backend compares the mount sources of the state volume and `BACKUP_DIR` (from `/proc/self/mountinfo`, as the document-mount check already does). If they share a source, or either is a network filesystem while the key is the generated one, the setup checklist shows "Your backup key is stored beside your backups; move it to a Docker secret" and the log says so on every start. The same warning covers `SECRET_KEY_FILE`, which has the same exposure today.

New monkeypatch points `backup.encrypt_stream(src, dst)` and `backup.decrypt_stream(src, dst)` (SQLite tests cannot rely on an `age` binary on the dev machine); the MAC is plain Python and tested for real.

**Tests:** the MAC over a known `SHA256SUMS` with a fixed identity; a tampered `SHA256SUMS`, a re-encrypted archive with a fresh identity, and a wrong key each refused; the key generated at first start with mode 0600 and never returned by any route; `backup-key show` and `rotate`; two identities, the older still decrypting; plain archives listed as unencrypted and refused without the typed override; every write path produces `.zip.age`. CI on real Postgres and the real `age`: download, scheduled, and pre-restore archives are age files; `age -d` with the key opens them; `restore.sh` restores one; flipping one byte makes both restore paths refuse.
...
**Classes after the renames:** 99 existing operations (100 less `POST /api/items/import`) plus 18 new routes (16 auth routes in section 5, 2 backup routes in section 2). The 99: public 1, read 33, write 45, admin 20. Two carry `metrics_ok`: `GET /api/stats/collection` (read) and `GET /api/metrics` (admin, so a read token gets 403). A `metrics` token on health gets the full body, like any principal. `@permission(..., fresh=True)` additionally requires the session's recent-password window (section 5); a token can never satisfy it. The full route-by-route table is in the appendix below.
...
- **Fresh (recent password):** the two exports, `GET /api/backup.zip`, `GET /api/backups/{name}`, deleting unencrypted archives, `POST /api/restore/inspect`, `POST /api/restore/{id}/run`, `POST /api/auth/tokens`, `DELETE /api/auth/tokens/{id}`, `DELETE /api/auth/sessions/{id}` for a session other than this one, `DELETE /api/auth/sessions` (sign out everywhere), `POST /api/auth/password`, `POST /api/auth/username`, purge and empty trash, and **every `PUT /api/settings`** (Codex round 2, A2, widened: without it, a stale session could set trash retention to one day and let the hourly clean-up delete for good, set `backup_keep` to 1 and prune the recovery points, or change the webhook format to silence alerts). Signing out of this session stays immediate.
...
| `status` | Claimed or not, the admin's name, last sign-in, failed sign-ins in the past 24 hours, live sessions and tokens with last use, the backup key's fingerprint and whether it has been saved. No secrets | none |
| `backup-key show` | Prints the backup key, for the owner to keep outside Cabinet | `backup_key_shown` |
| `backup-key rotate` | Adds a new identity first in `backup.key`; older archives stay readable | `backup_key_rotated` |
| `decrypt-archive` | Streams an encrypted archive from stdin to stdout after verifying it, for `restore.sh` | none |
...
| File | Change |
| --- | --- |
| `docker-compose.yaml` | No network pinning. The backend gets `PUBLIC_ORIGINS`, `SETUP_CODE`, `SETUP_CODE_FILE`, `BACKUP_KEY_FILE`, and `AUTH_INSECURE_HTTP` from `.env`, and a memory limit of 1024M, matching the stack file and the Argon2 budget in section 5 (Codex round 2, A1). The proxy gets `PUBLIC_ORIGINS` and `ALLOWED_HOSTS`. `ports: "${CABINET_PORT:-80}:80"`. Every service: `logging: {driver: json-file, options: {max-size: 10m, max-file: "3"}}` |
| `deploy/docker-stack.yaml` (an example, not the owner's) | `cabinet-internal` (internal): db, backend, proxy. `cabinet-egress`: **backend only**, with a comment never to replace it with a shared network. The proxy leaves egress and joins Traefik's network only when there is one (commented example). No subnets pinned. A commented `secrets:` block for `cabinet_setup_code` and `cabinet_backup_key`. The same log rotation. `ports: "${CABINET_PORT:?set CABINET_PORT}:80"` |
...
| `docs/deployment.md` | The shared-network advice at L303-310 is replaced with the rule that matters: nothing but nginx should reach the backend. The Traefik section adds the settings, the setup and backup-key secrets, the upgrade steps, and ingress mode. A new NAS section: export the data and backup folders only to the Swarm nodes, keep `root_squash`, never share them over SMB or to other hosts, and delete old unencrypted archives |
...
| 4 | **`cabinet_auth` schema and chain; credential-free archives; the marker; secrets cleared on restore** (sections 1 and 2) | Metadata FK test; dump/restore flags; TOC 422; every `recover()` row including a forged marker; restore keeps tokens | pytest; on local Postgres: fresh, a v0.29.1 database to head, auth chain up/down/up, a backup has no `cabinet_auth`, both restore paths leave it untouched |
| 5 | **Encrypted archives** (section 2): the key, `age` streaming, the MAC, every write path, inspect and restore decrypting, `restore.sh` and `backup.sh`, unencrypted archives listed and deletable, `backup-key` and `decrypt-archive` commands | The archive tests in section 2 | pytest; on the local stack: a download, a scheduled archive, and a safety backup are age files; `age -d` with the key opens one; both restore paths restore it; a flipped byte is refused |
...
| --- | --- | --- | --- |
| C1 | Tokens survive a password change or reset | **Taken.** Read and write tokens are revoked; metrics tokens kept and named | Sections 5, 6 |
...
Still outside Cabinet, and documented rather than built: Postgres's data directory and the photo and document volumes are plain files wherever the operator keeps them (section 2, "Data volumes").
...
| DELETE | `/api/backups/unencrypted` | admin, fresh | Deletes every plain `.zip` archive; audited and alerted |
