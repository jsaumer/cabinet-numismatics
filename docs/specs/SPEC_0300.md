...
| The key | One age identity (`AGE-SECRET-KEY-1...`). From `BACKUP_KEY_FILE` (a secret file) if set; otherwise generated at first start into `backup.key` (0600) on the state volume. A secret file must be readable by the app's user; the entrypoint never changes its ownership or mode, and an unreadable one stops startup before any backup is written. More than one identity is allowed for rotation: the first encrypts |
...
| Archive record (B3) | Every archive this Cabinet writes is recorded in `cabinet_auth.backup_ledger` (section 1), which no restore rewrites. Inspect says one of: "Made by this Cabinet on 18 September 2026", with "3 newer backups exist" when there are; "Not made by this Cabinet" (another machine, or an empty ledger on a new one). Restoring an archive older than the newest recorded one needs the typed confirmation `RESTORE OLDER` instead of `RESTORE`, and is audited and alerted |
...
- **Archive record (B3):** every write path adds a ledger row that survives a restore; the older of two genuine archives needs `RESTORE OLDER`, and plain `RESTORE` is refused for it; the newest needs neither; an archive from another key says "Not made by this Cabinet".
...
| `backup-key rotate` | Prepends a new identity to a generated `backup.key`; with `BACKUP_KEY_FILE`, prints the rotation steps and changes nothing | `backup_key_rotated` |
