...
| `POST` | `/api/auth/providers` | admin, recent password | `{preset, display_name, client_id, client_secret?, issuer?, tenant?, scopes?, logout_at_provider?, enabled?, dry_run?}`: `201` with the provider and `callback_urls`. `issuer` only for `custom` (https), `tenant` only for `microsoft`; `422` otherwise. `dry_run: true` fetches the discovery document only and saves nothing: `{ok, issuer, confirm, claims_supported_auth_time, prompt_login}` (`confirm` is what the sign-in will offer, by the same rule; a provider that publishes no `prompt_values_supported` qualifies), or `{ok: false, issuer, error}`. `409` for an issuer and client id already configured, a ninth provider, or two `PUBLIC_ORIGINS` entries on one host |
