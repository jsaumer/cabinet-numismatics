...
| `auth_config` | v0.33.0. One row (`id` is always 1): `password_sign_in_alerts`, `trusted_header_enabled` (with the four `TRUSTED_ASSERTION_*` variables, what turns header mode on), `updated_at`. Read on every use, never cached |
