...
def assert_clean(data: bytes, fmt: str) -> Image.Image:
    img = Image.open(io.BytesIO(data))
    img.load()
    assert img.format == fmt
    assert not photos.carries_metadata(img), sorted(img.info)
...
def test_the_pass_rewrites_what_carries_metadata_and_nothing_else():
    ...
    thumb = plant(root, "item-1/one_thumb.jpg", dirty("JPEG"))  # thumbnails are made clean
    working = plant(root, ".restore-new/item-9/nine.jpg", dirty("JPEG"))
    broken = plant(root, "item-3/broken.jpg", b"not an image at all")
    before = {p: (p.read_bytes(), p.stat().st_mtime_ns) for p in (tidy, thumb, working, broken)}
    assert not photos.marker_exists()

    found = photos.strip_existing()

    assert found == {"checked": 5, "rewritten": 3, "unreadable": 1, "failed": 0}
    ...
    assert photos.strip_existing()["rewritten"] == 0
